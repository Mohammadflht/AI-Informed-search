...
class SolveSudoku:
    ...
    def __init__(self, fpath):
        # two-way array for store the puzzle
        self.sud = []
    ...
    def goal_test(self):
        ...
        '''
        return 0 not in self.sud.flatten()

    def find_next_empty(self):
        '''
        Find the next empty cell in an unsolved board of Sudoku.
        The strategy is to find the cell that we think can have the minimum
...
if s.solve():
    print("solution: ")
    s.print()
    s.print(f"output.txt")
else:
    print("no solution")

finish = time.perf_counter()

print('time:\n', f"took {finish - start:.7f} seconds.")
ui_ai(s.sud.state)
